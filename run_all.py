...
from config import FED_DIR, NUM_CLIENTS
...
def prepare_for_federation(raw_path=None, n_clients=NUM_CLIENTS):
    ...
    le = LabelEncoder()
    le.fit(labels)
    joblib.dump(le, os.path.join(FED_DIR, "label_encoder.joblib"))
    ...
    scaler.fit(feat_arr)
    joblib.dump(scaler, os.path.join(FED_DIR, "feature_scaler.joblib"))
